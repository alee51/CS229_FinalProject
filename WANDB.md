...
3. **Project:** All runs go to the project **`cs229-metaworld`** (configurable in `baseline/train_config.yaml` or via your W&B account).
...
1. **Create a sweep** (from project root; uses `baseline/sweep.yaml`):

   ```bash
   wandb sweep baseline/sweep.yaml
   ```

   Copy the printed sweep ID (e.g. `entity/cs229-metaworld/abc123`).

2. **Run one or more agents** (each run executes one trial):

   ```bash
   wandb agent <entity>/cs229-metaworld/<sweep_id>
   ```
